...
<div align=center>
	<form action="documentOfficeVisit.jsp" method="post" id="formMain">
		...
		<p>Are you sure you want to document a <em>new</em> visit for <b><%=StringEscapeUtils.escapeHtml("" + (action.getUserName()))%></b>?</p>
		<input style="font-size: 150%; font-weight: bold;" type=submit
			value="Yes, Document <%=visitName%>">
	</form>
	<p> Click on an old office visit to modify:</p>
<%
	for (OfficeVisitBean ov : visits) {
		if (ov.isERIncident()) {
%>
	<a href="/iTrust/auth/hcp-uap/editOfficeVisit.jsp?ovID=<%=StringEscapeUtils.escapeHtml("" + (ov.getID()))%>"><%=StringEscapeUtils.escapeHtml("ER: "
							+ (ov.getVisitDateStr()))%></a><br />
<%
		} else if (!userRole.equals("er")) {
%>
	<a href="/iTrust/auth/hcp-uap/editOfficeVisit.jsp?ovID=<%=StringEscapeUtils.escapeHtml(""
