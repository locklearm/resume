...
public class AuthDAO {
	...
	public String addUser(Long mid, Role role, String password) throws DBException {
		...
		try {
			...
			pstmt.setLong(1, mid);
			String hashedPassword = DigestUtils.sha256Hex(password + DigestUtils.sha256Hex("" + mid));
	...
	public void resetPassword(long mid, String password) throws DBException {
		...
		try {
			...
			password = DigestUtils.sha256Hex(password + DigestUtils.sha256Hex("" + mid));
	...
	/**
	 * Returns the password of a particular MID
	 * 
	 * @param mid The MID of the user to look up.
	 * @return The {@link Password} of the user as a string.
	 * @throws DBException
	 * @throws iTrustException
	 */
	public String getPassword(long mid) throws DBException, iTrustException {
		Connection conn = null;
		PreparedStatement pstmt = null;
		try {
			conn = factory.getConnection();
			pstmt = conn.prepareStatement("SELECT Password FROM users WHERE MID=?");
			pstmt.setLong(1, mid);
			ResultSet rs;
			rs = pstmt.executeQuery();
			if (rs.next()) {
