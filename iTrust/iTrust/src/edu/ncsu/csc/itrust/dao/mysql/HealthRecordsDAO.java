...
public class HealthRecordsDAO {
	...
	public boolean add(HealthRecord record) throws DBException {
		if (record.getRecordID() == 0l) {
			System.out.println("Adding Record with id" + record.getRecordID());
			Connection conn = null;
			PreparedStatement ps = null;
			try {
				conn = factory.getConnection();
				ps = conn.prepareStatement("INSERT INTO personalhealthinformation(PatientID,Height,Weight,HeadCircumference,"
						+ "Smoker,SmokingStatus,HouseholdSmokingStatus,BloodPressureN,BloodPressureD,CholesterolHDL,CholesterolLDL,CholesterolTri,"
						+ "HCPID, OfficeVisitID) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			try {
				conn = factory.getConnection();
				ps = conn.prepareStatement("UPDATE personalhealthinformation SET PatientID=?,Height=?,Weight=?,HeadCircumference=?,"
						+ "Smoker=?,SmokingStatus=?,HouseholdSmokingStatus=?,BloodPressureN=?,BloodPressureD=?,CholesterolHDL=?,CholesterolLDL=?,CholesterolTri=?,"
						+ "HCPID=?, OfficeVisitID=? WHERE PersonalHealthInformationID=?");
				loader.loadParameters(ps, record);
				int numInserted = ps.executeUpdate();
				return (numInserted == 1);
			} catch (SQLException e) {
	 ...
	 */
	public void delete(long recordID) throws DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("DELETE FROM personalhealthinformation WHERE PersonalHealthInformationID=?");
