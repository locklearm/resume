...
public class HealthRecordFormValidator extends BeanValidator<HealthRecordForm> {
	...
	public void validate(HealthRecordForm bean) throws FormValidationException {
		...
		errorList.addIfNotNull(checkInt("Cholesterol HDL", bean.getCholesterolHDL(), 0, 89, true));
		errorList.addIfNotNull(checkInt("Cholesterol LDL", bean.getCholesterolLDL(), 0, 600, true));
		...
		errorList.addIfNotNull(checkNotZero("Head Circumference", bean.getHeadCircumference(), ValidationFormat.HEAD_CIRCUMFERENCE, true));
		errorList.addIfNotNull(checkInt("Smoker", bean.getIsSmoker(), 0, 10, false));
		errorList.addIfNotNull(checkInt("Household Smoking Status", bean.getHouseholdSmokingStatus(), 0, 10, false));
