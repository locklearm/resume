...
public class MedicalReportFilterTest extends TestCase {
	...
	public void testFilterByICDCode() {
		filter = new MedicalReportFilter(MedicalReportFilterType.DIAGNOSIS_ICD_CODE, "11.40", factory);
		List<PatientBean> res = filter.filter(allPatients);
		assertTrue(res.size() > 0);
	}
	
	public void testFilterByMissingDiagnosesICDCode() {
		filter = new MedicalReportFilter(MedicalReportFilterType.MISSING_DIAGNOSIS_ICD_CODE, "11.40", factory);
		List<PatientBean> res = filter.filter(allPatients);
		assertTrue(res.size() > 0);
	}
	...
	public void testFilterTypeFromStringBadName() {
		MedicalReportFilterType actual = MedicalReportFilter
				.filterTypeFromString("Never gonna give you up never gonna let you down");
		assertNull(actual);	
	}
