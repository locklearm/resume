...
public class AddPHRTest extends TestCase {
	...
	public void testDeleteSimple() throws Exception {
		HealthRecord record = hrDAO.getAllHealthRecords(2l).get(0);
		hrDAO.delete(record.getRecordID());
		HealthRecord record2 = hrDAO.getAllHealthRecords(2l).get(0);
		assertTrue(record.getRecordID() != record2.getRecordID());
	}
	
	public void testUpdateBadSimple() throws Exception {
		assertEquals(1, hrDAO.getAllHealthRecords(1L).size());
		HealthRecord hr = new HealthRecord();
		hr.setBloodPressureD(5);
		hr.setBloodPressureN(6);
